...
def parse_hypothesis(hyp, char_list):
    ...
    bpescore_list = hyp["att_bpescore"] # dean

    # convert to string
    tokenid = " ".join([str(idx) for idx in tokenid_as_list])
    token = " ".join(token_as_list)
    text = "".join(token_as_list).replace("<space>", " ")
    bpescore = " ".join(str(idx) for idx in bpescore_list) # dean

    return text, token, tokenid, score, bpescore
...
def add_results_to_json(js, nbest_hyps, char_list):
    ...
    for n, hyp in enumerate(nbest_hyps, 1):
        # parse hypothesis
        # dean: add bpescore
        rec_text, rec_token, rec_tokenid, score, bpescore = parse_hypothesis(hyp, char_list)
        ...
        out_dic["score"] = score
        out_dic["bpescore"] = bpescore # dean
...
def add_results_to_json_dean(js, nbest_hyps, name, char_list):
    """Add N-best results to json.
    ...
    for n, hyp in enumerate(nbest_hyps, 1):
        ...
        # save the decoder output into pt dean
        uttid = name + '-' + str(n-1)
        data_save[uttid] = dict()
        data_save[uttid]["decoder1"] = hyp["decoder_output"]
        data_save[uttid]["decoder2"] = hyp["decoder_output"]
        link = os.getcwd() + '/store_tmp/' + str(name) + '.pt' 
        new_js["feature_path"] = link
    pdb.set_trace()
    link_1 = os.getcwd() + '/store_tmp/'
    if os.path.exists(link_1) == False:
        os.makedirs(link_1)
    torch.save(data_save, link) 
        #datasave = dict{}
        #datasave["decoder_output"] = hyp["decoder_output"]

    return new_js

# * -------------------original one

def parse_hypothesis_origin(hyp, char_list):
    """Parse hypothesis.
...
def add_results_to_json_origin(js, nbest_hyps, char_list):
    """Add N-best results to json.
    ...
    for n, hyp in enumerate(nbest_hyps, 1):
        # parse hypothesis
        rec_text, rec_token, rec_tokenid, score = parse_hypothesis_origin(hyp, char_list)
