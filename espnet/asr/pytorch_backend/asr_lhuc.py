...
from espnet.asr.asr_utils import add_results_to_json_dean
...
from espnet.asr.pytorch_backend.asr_init import load_trained_model_sat
from espnet.asr.pytorch_backend.asr_init import load_trained_modules
from espnet.asr.pytorch_backend.asr_init import load_trained_modules_testadapt
from espnet.asr.pytorch_backend.asr_init import load_trained_modules_sat
...
from espnet.transform.transformation_lhuc import Transformation
from espnet.utils.cli_writers import file_writer_helper
from espnet.utils.dataset import ChainerDataLoader
from espnet.utils.dataset import TransformDataset
from espnet.utils.deterministic_utils import set_deterministic_pytorch
from espnet.utils.dynamic_import import dynamic_import
from espnet.utils.io_utils_lhuc import LoadInputsAndTargets
from espnet.utils.io_utils_lhuc import LoadInputsAndTargets_addprob
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        # dean 2021/11/29: change the long to float
        # pdb.set_trace()
        # print(ilens)
        ys_pad = pad_list(
            [
...
def train(args):
    ...
    # pdb.set_trace()
    # Dean Speaker adaptation
    # case 1: test adapt -> train from the SI baseline system, need to load the parameters and freeze them
    # case 2: sat -> train sat from the SI baseline system, but no need to freeze the SI-parameters
    # case 3: sat-scratch -> train sat from scratch
    if (args.speaker_adaptation == "test-adapt") and args.num_encs == 1: # test adaptation, only load the parameters of "no_lhuc" from the baseline models
        model = load_trained_modules_testadapt(idim_list[0], odim, args)
        logging.info("Using test adaptation. It contains lhuc layers")
    elif (args.speaker_adaptation == "sat") and args.num_encs == 1: # sat, only load the parameters of "no_lhuc" from the baseline models
        model = load_trained_modules_sat(idim_list[0], odim, args)
        logging.info("Using SAT adaptation. It contains lhuc layers")
    elif (args.speaker_adaptation == "sat-scratch") and args.num_encs == 1: # sat, train it from scratch
        model = load_trained_modules(idim_list[0], odim, args)   
        logging.info("Using SAT from scratch adaptation. It contains lhuc layers")
    else: # no speaker adaptation
        if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
            model = load_trained_modules(idim_list[0], odim, args)
            logging.info("Using standard version. No lhuc layers.")
        else:
    ...
    if args.opt == "adadelta":
        ...
        optimizer = torch.optim.SGD(model_params, lr=args.sgd_lr)
    elif args.opt == "noam":
    ...
    # dean
    # 2021/11/29: update the y label from [1, len] to [2, len], the 2-nd dim is the attention probability
    if args.bpescore == True:
        load_tr = LoadInputsAndTargets_addprob(
            mode="asr",
            load_output=True,
...
def recog(args):
    ...
    set_deterministic_pytorch(args)
    # dean oct/20
    if args.speaker_adaptation == "sat":
        model, train_args = load_trained_model_sat(args.model, training=False)
    else:
