...
def get_nolhuc_modules(model_state_dict, model_flag):
    """Get non-lhuc modules in model state dict.

    Args:
        model_state_dict (Dict): Pre-trained model state dict.

    Return:
        new_mods (List): No-LHUC-module list.
    ...
    mods_model = list(model_state_dict.keys()) # encoder.embed.conv.0.weight


    for key in mods_model:
        if "lhuc" in key:  # select the module list without "lhuc"
            lhuc_mods += [key]
        else:
            pretrained_mods += [key] 
    ...
    #     logging.info(
    #         f"model: {model_flag}."
    #         f"lhuc mods: {lhuc_mods}."
    #     )

    return pretrained_mods

def get_nolhuc_and_dean_modules_decode_reducedim(model_state_dict, model_flag):
    """Get non-lhuc modules in model state dict.
    ...
    for key in mods_model:
        if "lhuc" in key or "dean" in key:  # select the module list without "lhuc"
            lhuc_mods += [key]
        else:
            pretrained_mods += [key] 
...
def load_trained_model_sat(model_path, training=True):
    """Load the trained model for recognition. SAT need to reinitialize the lhuc layers.
    ...
    # new model 
    # remove the lhuc layers
    train_args_main = train_args
    train_args_main.lhuc_layers = ""
    model_class = dynamic_import(model_module)
    model = model_class(idim, odim, train_args_main)

    def print_new_keys(state_dict, modules, model_path):
        logging.info(f"Loading {modules} from SAT baseline: {model_path}")
    ...
    if model_path is not None:
        if os.path.isfile(model_path):
            model_state_dict = get_trained_model_state_dict(
                model_path, "transducer" in train_args.model_module
            )

            si_modules = get_nolhuc_and_dean_modules_decode_reducedim(model_state_dict, "SI model")  # get nolhuc layers from the pretrained SI models
            
            partial_state_dict = get_partial_state_dict(model_state_dict, si_modules) # check whether the si modules is in the SI models

            if partial_state_dict:
                if transfer_verification(
                    main_state_dict, partial_state_dict, si_modules
                ):
                    print_new_keys(partial_state_dict, si_modules, model_path)
...
# dean add load_trained_modules from here
def load_trained_modules_testadapt(idim, odim, args, interface=ASRInterface):
    """Load ASR/MT/TTS model with pre-trained weights for specified modules.
    ...
    def print_new_keys(state_dict, modules, model_path):
        logging.info(f"Loading {modules} from SI baseline: {model_path}")

        for k in state_dict.keys():
            logging.warning(f"Overriding module {k}")

    model_si_path = args.model_si_path
    ...
    # pdb.set_trace()
    if model_si_path is not None:
        if os.path.isfile(model_si_path):
            model_state_dict = get_trained_model_state_dict(
                model_si_path, "transducer" in args.model_module
            )
            #pdb.set_trace()
            si_modules = get_nolhuc_modules(model_state_dict, "SI model")  # get nolhuc layers from the pretrained SI models
            ...
            if partial_state_dict:
                if transfer_verification(
                    ...
                    print_new_keys(partial_state_dict, si_modules, model_si_path)
                    main_state_dict.update(partial_state_dict)
        else:
            logging.error(f"Specified model was not found: {model_si_path}")
            exit(1)

    main_model.load_state_dict(main_state_dict)
    # freeze
    main_model, model_params = freeze_modules(main_model, si_modules)
    # pdb.set_trace()
    return main_model

# dean add load_trained_modules from here SAT (No freeze)
def load_trained_modules_sat(idim, odim, args, interface=ASRInterface):
    """Load ASR/MT/TTS model with pre-trained weights for specified modules.
