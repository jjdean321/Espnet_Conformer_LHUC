...
class Transformation(object):
    ...
    def __call__(self, xs, uttid_list=None, **kwargs):
        ...
        # dean we need to extrct the last dim of input feature 
        xs_input = xs
        spkid = [np.expand_dims(xs_input[i][:,-1], axis=1) for i in range(len(xs_input))]
        xs = [xs_input[i][:,0:-1] for i in range(len(xs_input))] # dim - 1
        ...
        #pdb.set_trace()
        # step2 dean, we need to concate the spkid to the origin dim
        xs_output = [np.concatenate((xs[i], spkid[i]), axis=1) for i in range(len(xs))] 
        xs = xs_output
