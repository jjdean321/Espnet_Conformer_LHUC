...
class LoadInputsAndTargets_addprob(object):
    """Create a mini-batch from a list of dicts
            ...
            if self.load_output:
                ...
                for idx, inp in enumerate(info["output"]):
                    if "tokenid" in inp:
                        # ======= Legacy format for output =======
                        # {"output": [{"tokenid": "1 2 3 4"}])
                        # dean {"output": [{"tokenid": "1 2 3 4"}, bpescore])
                        # ----- origin
                        # x = np.fromiter(
                        #     map(int, inp["tokenid"].split()), dtype=np.int64
                        # )
                        ...
                        # x_dean = np.array()
                        x_dean1 = np.fromiter(
                            map(float, inp["tokenid"].split()), dtype=np.float32
                        )
                        x_dean1 = np.append(x_dean1, -1) # pad the <eos> prob
                        x_dean2 = np.fromiter(
                            map(float, inp["bpeattprob"].split()), dtype=np.float32
                        )
                        x_dean = np.array([x_dean1, x_dean2]).transpose() # 35, 2   
                        # pdb.set_trace()                
                        # ----- dean
                        ...
                        #    "filetype": "hdf5",
                        #    "name": "target1", ...}], ...}
                        x_dean = self._get_from_loader(
                            filepath=inp["feat"], filetype=inp.get("filetype", "mat")
                        )

                    y_feats_dict.setdefault(inp["name"], []).append(x_dean)
