...
def get_parser(parser=None, required=True):
    ...
    parser.add_argument(
        "--sgd-lr", default=1e-4, type=float, help="Learning rate of SGD"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="",
        type=lambda s: [str(mod) for mod in s.split(",") if s != ""],
        help="List of lhuc layers to initialize, separated by a comma.",
    )
    parser.add_argument(
        "--speaker-num",
        default=1,
        type=int,
        help="Number of speakers in the lhuc.",
    )
    parser.add_argument(
        "--speaker-adaptation",
        type=str,
        default=None,
        help="Use speaker adaptation.",
    )   
    parser.add_argument(
        "--model-si-path",
        default=None,
        type=str,
        help="Pre-trained ASR-SI model's path to initialize SD model.",
    )
    parser.add_argument(
        "--data-use-spkid",
        type=strtobool,
        default=False,
        help="The flag to switch to use lhuc. Where its data input has one additional dimension for spkid",
    )
    ...
    #     type=strtobool,
    #     default=True,
    #     help="The flag to modify the loss function with weight bpescore",
    # )
    # parser.add_argument(
    #     "--lhuc-dim",
    #     default=0,
    #     type=int,
    #     help="The dimension of lhuc parameters.",
    # )
    return parser
...
def main(cmd_args):
    ...
    if args.num_spkrs == 1:
        if args.backend == "chainer":
            ...
            # whether data contains spkid
            if args.data_use_spkid == True:
                from espnet.asr.pytorch_backend.asr_lhuc import train
                train(args)
                logging.info("Using lhuc_modified verision. The data input feature contains spkid information")
            else:
